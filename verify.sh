...
if sqlite3 "$HOME/Library/Messages/chat.db" "select count(*) from message;" >/dev/null 2>&1; then
  ...
  no "Full Disk Access — cannot read ~/Library/Messages/chat.db (message scanning will return nothing)"
fi
...
if launchctl print-disabled system 2>/dev/null | grep -q '"com.openssh.sshd" => enabled'; then
  ...
  no "Remote Login is off — Settings → General → Sharing → Remote Login"
else
...
if pmset -g 2>/dev/null | awk '/^ *sleep/{print $2}' | grep -q '^0$'; then
  ...
  no "system sleeps — 'sudo pmset -a sleep 0' or overnight loops are unreliable"
fi

# ── phase 3 · core CLIs ──────────────────────────────────────────────────────
head_ "phase 3 · core CLIs"
for b in brew node npm git gh jq tmux python3 sqlite3; do
  if command -v "$b" >/dev/null 2>&1; then ok "$b"; else no "$b missing"; fi
done

if command -v gh >/dev/null 2>&1; then
  who="$(gh api user -q .login 2>/dev/null)"
  if [ -z "$who" ]; then
    no "gh is not authenticated (gh auth login)"
  elif [ -n "$GH_USER" ] && [ "$who" != "$GH_USER" ]; then
    no "gh is authenticated as '$who' but \$GH_USER is '$GH_USER' — this blocks deploys"
  else
...
if [ -z "$TSBIN" ]; then
  no "no tailscale CLI — install the standalone app, not the App Store build"
elif [ "$QUICK" = "1" ]; then
  ...
  if "$TSBIN" serve status 2>/dev/null | grep -q 'ts\.net'; then
    ...
    no "no tailscale serve mappings — phone surfaces will be unreachable"
  fi
fi
...
if command -v claude >/dev/null 2>&1 || [ -x "$HOME/.local/bin/claude" ]; then
  ...
  no "claude not on PATH — check ~/.local/bin in .zshrc"
fi
...
if [ -r "$SL" ]; then
  ...
  if grep -q "\$'" "$SL"; then
    ...
    no "status line colors are not \$'...' — they will print literally"
  fi
  command -v jq >/dev/null 2>&1 \
    && ok "jq present — status line can parse its input" \
    || no "jq missing — status line will render blank"
else
  no "$SL missing"
fi
...
if [ -f "$HOME/.claude/settings.json" ]; then
  if command -v jq >/dev/null 2>&1; then
    if jq -e '.statusLine.command' "$HOME/.claude/settings.json" >/dev/null 2>&1; then
      ...
      no "settings.json has no statusLine block"
    fi
  else
    skip "settings.json present (no jq to inspect it)"
  fi
else
  no "~/.claude/settings.json missing"
fi
...
for f in "$HOME/.claude/CLAUDE.md" "$HOME/.claude/USER.md"; do
  if [ ! -f "$f" ]; then
    no "$(basename "$f") missing — $f"
  else
    n="$(wc -l < "$f" | tr -d ' ')"
    if [ "$n" -le 200 ]; then ok "$(basename "$f") — $n lines"
    else no "$(basename "$f") — $n lines, over the 200 cap"; fi
  fi
done
...
if [ "$QUICK" = "0" ] && command -v claude >/dev/null 2>&1; then
  ...
  else no "no MCP servers connected"; fi
else
...
if [ -z "$ORG" ]; then
  ...
  if [ "${loaded:-0}" -eq 0 ]; then
    ...
    if [ -n "$bad" ]; then
      for l in $bad; do no "$l is loaded but last exited non-zero"; done
    else
